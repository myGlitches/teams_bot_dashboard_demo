...
import { totalSessionsData30Days } from "@/lib/data/comprehensiveFulldata"
...
export const TotalSessionsGraph = () => {
  const {
    startDate,
    endDate,
    getGraphFilters,
  ...
  const filters = getGraphFilters('totalSessions')
  ...
  const handleProductChange = (values: string[]) => {
    setGraphProduct('totalSessions', values)
  }

  const handleStoreIdChange = (values: string[]) => {
    setGraphStoreId('totalSessions', values)
  }
  ...
  const data = useMemo(() => {
    return totalSessionsData30Days.filter((d) => {
  ...
  const traces: Data[] = useMemo(() => {
    ...
    if (selectedDimension.length === 0) {
      // Group by date and sum all sessions
      const aggregatedData = data.reduce((acc, curr) => {
        if (!acc[curr.date]) {
          acc[curr.date] = 0
        }
        acc[curr.date] += curr.totalSessions || 0
        return acc
  ...
  return (
    <div className="group relative overflow-hidden bg-gradient-to-br from-white to-blue-50/30 rounded-3xl border border-white/60 shadow-2xl shadow-blue-500/10 hover:shadow-blue-500/20 transition-all duration-500 hover:scale-[1.02] w-full">
      <div className="absolute top-0 right-0 h-40 bg-gradient-to-br from-blue-500/5 to-indigo-500/10 rounded-full -translate-y-20 translate-x-20"></div>
        ...
        <div className="relative">
          <Plot
            ...
            layout={{
              ...
              yaxis: { 
                title: { text: "Sessions", font: { size: 12, color: "#64748b" } },
                gridcolor: "#f1f5f9",
                tickfont: { size: 11, color: "#64748b" }
              },
              hovermode: "x unified",
