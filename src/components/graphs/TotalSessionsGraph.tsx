...
import { totalSessionsData } from "@/lib/data/dummyData"
...
export const TotalSessionsGraph = () => {
  ...
  const data = useMemo(() => {
    return totalSessionsData.filter((d) => {
  ...
  const traces: Data[] = useMemo(() => {
    ...
    // Create traces for each combination of active filters
        ...
        productsToShow.forEach(product => {
          storeIdsToShow.forEach(storeId => {
            ...
            if (filteredData.length > 0) {
              const hasValidData = filteredData.some(d => d.totalSessions !== undefined)
              if (hasValidData) {
                traces.push({
                  x: filteredData.map(d => d.date),
                  y: filteredData.map(d => d.totalSessions!).filter(val => val !== undefined),
                  type: "scatter",
                  mode: "lines+markers",
